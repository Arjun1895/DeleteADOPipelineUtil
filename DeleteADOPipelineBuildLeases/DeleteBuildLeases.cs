    ...
    class DeleteBuildLeases
    {
        static async Task Main(string[] args)
        {
            try
            {
                ...
                var azureDevOpsApiClient = new AzureDevOpsApiClient(personalAccessToken, organization, project);

                List<Build> builds = await azureDevOpsApiClient.GetAllBuilds(buildDefinitionId);
                ...
                if (deletePipelineConfirmation == "y") {
                    await azureDevOpsApiClient.DeletePipeline(buildDefinitionId);
